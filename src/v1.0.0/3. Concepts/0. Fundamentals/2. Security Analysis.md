...
1. The system must be as decentralized as the main chain. This means node costs (for both the main chain and the layer-2 system combined) should not be meaningfully higher (than just the main chain). Or, more specifically, the marginal cost of each layer-2 transaction should be lower than the marginal cost of each layer-1 transaction.
...
1. Due to data withholding, sidechain degenerate to being permissioned: a cabal of sidechain block producers can create a block that other sidechain block producers can never build upon.
1. Node costs increase proportionally to the number of sidechain transactions.
1. Any number of sidechains may be deployed in parallel, so throughput can be greatly increased.
1. Sidechains may have any execution model. As such, they have have a different—and more scalable—execution model than the main chain.
1. Additional concerns:
